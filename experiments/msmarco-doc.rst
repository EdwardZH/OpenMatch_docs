...
~~~~~~~~~
We provide dense retriever codes (optimized ANCE) in OpenMatch for
MSMARCO Document and MSMARCO-like datasets.

In such dataset, several files must be provided: - documents -
``[document collection.tsv]``: each line contains
``[passage id]\t[text]\n`` for the document texts. ``[passage id]``\ s
...
``[query id]`` is also integers. - ``[custom qrels.tsv]``:
...
You can specify a pytorch checkpoint and use it to inference the
embeddings of the documents or queries.
...
``--inference_type document`` and removing ``--save_prefix``, you can
